...
//   5. Actualiza el deal en Bancos: alquiler mensual + historicoCambiosPrecios
//      (el two-way sync de Airtable se encarga de llevarlo al Gestor)
//
...
// --- STEP 8: Actualizar deal en Bancos (two-way sync lo lleva al Gestor) ---
...
if (linkedDeal && linkedDeal.length > 0) {
    ...
    await dealsTable.updateRecordAsync(linkedDeal[0].id, {
        'fldZE4Q0citWseonB': nuevoPrecio,      // alquiler mensual
        'fld0HrIz7ZblScrqk': nuevoAviso         // historicoCambiosPrecios
    });
    console.log(`Deal actualizado en Bancos (two-way sync → Gestor):`);
    console.log(`  alquiler mensual: €${nuevoPrecio}`);
    console.log(`  historicoCambiosPrecios: ${nuevoAviso.split('\n').length} líneas`);
} else {
    console.log('Balance sin linkDeal, actualización de deal omitida');
}
