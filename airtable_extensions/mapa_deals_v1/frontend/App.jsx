...
// Lee deals en vivo, filtra activos, filtro por mes de cierre + facetas (canal/producto/tipo),
// mapa SVG offline con detalle por zoom, y panel de análisis (deals + renta media) por selección.
...
const NO_FACETS = {canal: null, producto: null, tipo: null};
...
function MapaDeals({table}) {
  ...
  const [facets, setFacets] = useState(NO_FACETS); // {canal, producto, tipo}
  ...
  // monthFiltered: solo filtro temporal (base estable para las barras de facetas).
  ...
  // filtered: además, las facetas activas. Es lo que pinta el mapa y los KPIs.
  ...
  const selectionDeals = useMemo(() => {
    ...
    const map = {ccaa: agg.byCCAA, provincia: agg.byProvince, municipio: agg.byMunicipio, cp: agg.byCP, agencia: agg.byAgencia}[selection.kind];
  ...
  const selectAgencia = useCallback((name) => setSelection({kind: 'agencia', key: name, title: name}), []);
  ...
  const anyFacet = facets.canal || facets.producto || facets.tipo;
      ...
      <div className="flex-1 flex min-h-0">
        <aside className="w-72 shrink-0 bg-paper border-r border-line overflow-y-auto p-4 space-y-5">
          ...
          <div className="border-t border-line pt-4 space-y-3">
            ...
            <Facetas title="Canal" entries={facetStats.byCanal} active={facets.canal} onToggle={(v) => toggleFacet('canal', v)} />
            <Facetas title="Producto" entries={facetStats.byProducto} active={facets.producto} onToggle={(v) => toggleFacet('producto', v)} />
            <Facetas title="Tipo de contrato" entries={facetStats.byTipo} active={facets.tipo} onToggle={(v) => toggleFacet('tipo', v)} />
          </div>
          ...
          <div className="border-t border-line pt-4">
            <div className="text-[11px] font-semibold text-navy mb-2">Top agencias</div>
            <RankingAgencias byAgencia={agg.byAgencia} total={filtered.length} selectedKey={selection && selection.kind === 'agencia' ? selection.key : null} onSelect={selectAgencia} />
          </div>
        </aside>
